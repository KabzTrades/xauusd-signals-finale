...
# ---------------------------------------------------------------------------
# Module 9 - Signal Journal
#
# GitHub Actions runners are ephemeral - nothing survives between runs
# unless it's committed back to the repo. This CSV is that persistence
# layer: every evaluation (fired or not) gets one row, and the workflow
# commits the file after each run. This is also the prerequisite for the
# taper mechanism flagged back in Module 4 - "how many signals fired
# today" now has somewhere to actually live.
# ---------------------------------------------------------------------------
JOURNAL_PATH = "signal_journal.csv"
JOURNAL_FIELDS = [
    "timestamp_utc", "instrument", "fired", "direction", "confidence",
...
def append_journal_entry(evaluation: dict, price: float, path: str = JOURNAL_PATH) -> None:
    """
    One row per evaluation, fired or not - this is what makes signal
    frequency and outcomes reviewable later instead of vanishing when the
    Actions runner shuts down. Writes the header only if the file is new.
    """
    ...
    row = {
        ...
        "confidence": evaluation["confidence"] if evaluation["confidence"] is not None else "",
        "entry": levels["entry"] if levels else price,
        "stop_loss": levels["stop_loss"] if levels else "",
        "take_profit": levels["take_profit"] if levels else "",
...
def main() -> None:
    ...
    append_journal_entry(evaluation, price)
